...
function assembleStreamToJSON(raw) {
  ...
  for (const line of raw.split('\n')) {
    ...
    try {
      ...
      if (event.type === 'message_start') {
        ...
        // Anthropic reports mid-stream failures this way, after the response has
        // already started with a 200. Dropping it would leave a record that looks
        // like a successful empty answer.
        message.error = event.error || { type: 'error', message: 'stream failed' };
      }
    } catch {}
  }
  ...
  for (const idx of Object.keys(contentBlocks).sort((a, b) => Number(a) - Number(b))) {
    ...
    if (block.type === 'text') {
      ...
      // The block never closed, so the arguments were never parsed into input.
      if (block.partial_json) contentItem.partial_json = block.partial_json;
    }

    message.content.push(contentItem);
  }

  // Nothing recognisable was assembled. Returning null lets the caller keep the
  // raw stream instead of recording an empty message.
  const hasUsage = Boolean(message.usage.input_tokens || message.usage.output_tokens);
  if (!message.id && message.content.length === 0 && !message.error && !hasUsage) {
    return null;
  }
...
function* parseAnthropicStreamChunks(line) {
  yield* parseStreamLine(line, function* (event) {
    if (event.type === 'content_block_delta') {
      ...
      const label = detail.type && detail.message
        ? `${detail.type}: ${detail.message}`
        : (detail.message || detail.type || 'stream error');
      yield { type: 'error', error: label };
    }
  });
}
...
function tryExtractTokenUsage(body) {
  ...
  if (body.usage && body.usage.prompt_tokens !== undefined) {
    return {
      ...
      total_tokens: tokenCount(body.usage.total_tokens),
    };
  }
  ...
  if (body.usage && typeof body.usage.input_tokens === 'number' && body.usage.total_tokens === undefined) {
    return {
      ...
      total_tokens: tokenCount(body.usage.input_tokens) + tokenCount(body.usage.output_tokens),
    };
  }
